...
def to2Gis(adress):
    """Преобразует адрес в формат, понятный API 2ГИС.

    Args:
        adress (str): Адрес в формате "ул. [Название улицы], д. [Номер дома]".

    Returns:
        tuple: Кортеж из названия улицы и номера дома.
    """
...
def lenWay(pickup_location, dropoff_location):
    """Вычисляет длину маршрута между двумя точками с помощью API 2ГИС.

    Args:
        pickup_location (str): Адрес места начала поездки.
        dropoff_location (str): Адрес места окончания поездки.

    Returns:
        int: Длина маршрута в метрах.
    """
